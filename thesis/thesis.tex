...
Linux 平台上的I/O模型经历了几个发展阶段：阻塞，非阻塞，多路复用，信号驱动，异步。其中前三种均属于
...
如何降低异步I/O带来的编程复杂度，是软件开发者不可忽视的问题之一。针对这个问题，工业界提出了一个
解决方案：通过\textbf{协程}与异步I/O结合进而降低编程难度。\par

协程（英文 coroutine）一词最早出自于上个世纪的程序员马尔文·康威，关于协程的论文最早在
1963\cite{paper1}年发表。协程可以被理解为是一个可以随时挂起和恢复运行的函数。它可以通过\textbf{yield}
