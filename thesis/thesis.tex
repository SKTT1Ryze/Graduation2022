...
这里出现一个问题：谁来对已经挂起的Future进行恢复操作呢？答案是将会在下文介绍的执行器Executor。\par

Future在Rust中是以接口的形式定义的，当一个结构体A实现了Future接口接口，则可以称A为一个Future。
...
然后再写入到另一个文件当中，这时候把读操作和写操作分别看作是一个Future的话，那么读Future和写Future就是
...
async/await解决了传统异步编程的诸多痛点，同时能让开发者像写同步代码一样编写异步代码，
极大地降低了异步程序开发的难度。
这也是为什么Rust 2018版本稳定了async/await语法后，
...
来管理和运行异步任务。负责这个工作的组件，被称为Executor（执行器）。\par
