...
基于异步I/O和协程设计并分别使用C++和Rust语言实现了两款高性能的异步编程框架\textbf{Kuro}和\textbf{Emma}。
Kuro和Emma框架底层使用了Linux平台上先进的io\underline{~}uring技术，并兼容C++和Rust语言的官方协程标准，
相比传统的同步I/O和多线程编程模式，具有更轻量，更高性能的优势。\par

在功能上，Kuro和Emma框架提供简单易用的接口，
向上层应用开发者提供构建高性能存储和网络应用的能力。
在性能上，Kuro和Emma框架和当前开源生态中主流的性能最高的异步编程框架相比，
具备更高的性能。
此外，通过对Kuro和Emma框架和传统的同步I/O与多线程编程模式进行性能对比测试，
...
The Kuro and Emma frameworks use the advanced io\underline{~}uring technology on the Linux platform at the bottom,
...
Functionally, the Kuro and Emma frameworks provide easy-to-use interfaces and provide upper-layer application developers
with the ability to build high-performance storage and network applications.
In terms of performance, the Kuro and Emma frameworks have higher performance
...
In addition, through the performance comparison test of Kuro and Emma frameworks and traditional synchronous I/O and multi-threaded programming modes,
...
目前，国内外对异步编程框架的研究主要体现了当前开源社区中的几个主流开源项目中，
...
下一章将进入正题，介绍和分析异步编程框架的整体系统设计。\par
...
基于以上设计，可以看到io\underline{~}uring和协程的概念被封装到了
...
在第二章介绍io\underline{~}uring提到，用户可以通过往提交队列添加SQE来提交I/O请求和从完成队列收割CQE
...
\begin{enumerate}
  \item 从测试结果中可以看到，无论是单核场景还是多核场景，
    实现的框架Kuro和Emma在异步文件读写上比Tokio，Go等目前主流的开源异步编程框架具有更好的性能。
    Kuro和Emma底层使用了io\underline{~}uring作为异步文件I/O接口，而其他框架底层使用线程池模拟等方式
    实现异步文件I/O，因此该测试结果验证了io\underline{~}uring接口的高性能；
  \item 从测试结果中发现，相比于单核场景，在多核场景下Kuro和Emma的性能有较大幅度提升，
    这表示实现的框架Kuro和Emma同样可以充分利用到现代CPU的多核架构，发挥更极致的性能；
...
\begin{enumerate}
  \item 从测试结果中可以看到，实现的框架Kuro和Emma在参与性能测试的各个异步编程框架中
  ...
  \item 对比C=20和C=200的测试结果，发现在并发数较大时，Kuro和Emma的性能优势更加明显，
...
最后，对实现的两个异步编程框架Kuro和Emma进行了性能测试，并完成了对测试结果的分析说明。\par
...
\begin{enumerate}
  ...
  \item \textbf{更规范的性能测试}，在性能测试环节中，存在一些可能对实验结果产生影响的因素，
    ...
    获得更准确的结果；
  \item \textbf{系统实现细节的优化}，在Kuro的Emma的实现过程中，发现了许多性能优化点，
