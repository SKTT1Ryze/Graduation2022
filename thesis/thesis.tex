...
本章首先介绍了课题相关的研究背景和技术发展趋势，
然后分析了当前系统并发编程领域面临的问题和挑战，
接着介绍了国内外针对异步编程框架进行的相关研究工作，
...
操作系统内核为每个线程分配独立的栈空间，
使得每个线程可以独立地并行执行，能成倍提高程序运行的效率。同时，每个线程可以占据独立的CPU（中央处理器）核，
充分利用现代CPU的多核架构\cite{hong2011efficient}。比如，一个用户程序运行在一个拥有4核的CPU芯片上，创建4个线程，
...
1963年发表。协程可以被理解为是可以随时挂起和恢复运行的函数。它可以通过挂起
来让出当前CPU，然后运行其他协程；它也可以来恢复执行。当一个协程被恢复的时候，
它会从上次挂起的位置继续执行。协程和线程类似，属于一种并发编程模式。\par
...
当创建了多个协程时，从用户视角来看可以认为是多个协程在并发运行，
...
\begin{enumerate}[label={(\arabic*)}]
  ...
  \item 修改AIO接口会引入新的问题，或者会使得现有问题变得更加严重，比如会使得现有API（应用程序接口）变得复杂，难以使用。
\end{enumerate}

以上种种因素使得Linux社区迎来了新时代的异步I/O接口：io\underline{~}uring。
io\underline{~}uring异步接口于Linux 5.1版本开始提供，统一了整个Linux平台上的异步I/O框架。\par
...
\begin{enumerate}[label={(\arabic*)}]
  \item 易于使用，难以滥用；
  \item 可扩展的，该接口不仅仅用于块存储，还可用于非块存储和网络通信；
  \item 功能丰富；
  \item 高性能。
\end{enumerate}
...
\begin{enumerate}[label={(\arabic*)}]
  ...
  \item 接口易用。提供用于构建网络应用和存储应用所需的简单易用的功能接口，例如TCP（传输控制协议）协议的网络连接；
...
但是Future表示的是一个将来值，需要特殊的方法才能将多个Future组合起来。\par
...
这个将来值具备挂起当前协程和恢复后获得返回值的功能，如此Op模块的功能得以实现。\par
...
使用参与性能测试的各框架编写一个简单的HTTP（超文本传输协议）服务器应用，该服务器需要监听某个固定的IP地址和端口，
